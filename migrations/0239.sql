...
SELECT
	...
	sc.amount * p.carbs AS carbs,
	sc.amount_aggregated * p.carbs AS carbs_aggregated,
	p.sugar AS product_sugar,
	sc.amount * p.sugar AS sugar,
	sc.amount_aggregated * p.sugar AS sugar_aggregated,
	p.fat AS product_fat,
	sc.amount * p.fat AS fat,
	sc.amount_aggregated * p.fat AS fat_aggregated,
	p.fatacid AS product_fatacid,
	sc.amount * p.fatacid AS fatacid,
	sc.amount_aggregated * p.fatacid AS fatacid_aggregated,
	p.protein AS product_protein,
	sc.amount * p.protein AS protein,
	sc.amount_aggregated * p.protein AS protein_aggregated,
	p.salt AS product_salt,
	sc.amount * p.salt AS salt,
	sc.amount_aggregated * p.salt AS salt_aggregated,
...
BEGIN
	...
	carbs = carbs / IFNULL((SELECT factor FROM quantity_unit_conversions_resolved WHERE product_id = NEW.id AND from_qu_id = OLD.qu_id_stock AND to_qu_id = NEW.qu_id_stock LIMIT 1), 1.0),
	sugar = sugar / IFNULL((SELECT factor FROM quantity_unit_conversions_resolved WHERE product_id = NEW.id AND from_qu_id = OLD.qu_id_stock AND to_qu_id = NEW.qu_id_stock LIMIT 1), 1.0),
	...
	fatacid = fatacid / IFNULL((SELECT factor FROM quantity_unit_conversions_resolved WHERE product_id = NEW.id AND from_qu_id = OLD.qu_id_stock AND to_qu_id = NEW.qu_id_stock LIMIT 1), 1.0),
	...
	salt = salt / IFNULL((SELECT factor FROM quantity_unit_conversions_resolved WHERE product_id = NEW.id AND from_qu_id = OLD.qu_id_stock AND to_qu_id = NEW.qu_id_stock LIMIT 1), 1.0),
